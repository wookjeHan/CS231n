...
def content_loss(content_weight, content_current, content_original):
    ...
    M_l=H_l*W_l
    c_c=content_current.view(C_l,M_l)
    c_o=content_original.view(C_l,M_l)
    loss=torch.sum((c_c-c_o)**2)*content_weight
    return loss
    pass
...
def gram_matrix(features, normalize=True):
    ...
    GM=torch.zeros(N,C,C)
    flattenF=features.view(N,C,H*W)
    GM=flattenF.bmm(flattenF.transpose(1,2))
    if(normalize):
        GM/=H*W*C
    return GM
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        a=style_layers[i]
        gl=gram_matrix(feats[a])
        sLoss+=style_weights[i]*(((gl-style_targets[i])**2).sum())
    pass
    return sLoss
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def tv_loss(img, tv_weight):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    _,_,H,W=img.shape
    img2=torch.roll(img,-1,2)
    img2[:,:,H-1,:]=img[:,:,H-1,:]

    img3=torch.roll(img,-1,3)
    img3[:,:,:,W-1]=img[:,:,:,W-1]
    sum1=torch.sum((img2-img)**2)
    sum2=torch.sum((img3-img)**2)
    loss=tv_weight*(sum1+sum2)
    return loss
